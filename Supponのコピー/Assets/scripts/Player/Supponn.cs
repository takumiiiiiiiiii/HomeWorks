...
public class Supponn : MonoBehaviour
{
    ...
    [SerializeField] private float Bw;//大きさ
    public bool dw,up;//上昇,下降
    private bool NM;//動けなくする
    private int NMcnt;//
    public float playerX, playerZ;//プレイヤーの座標を保存
    Vector3 Npos; //最初の座標
    ...
    void FixedUpdate()
    {
        ...
        if (NM == false)//移動できる状態か
        {
            ...
            {
                //rb.velocity = new Vector3(rb.velocity.x, -spr,rb.velocity.z);
