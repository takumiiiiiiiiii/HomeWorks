...
public class ShotP2 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (countdown <= 0)
        {
            ...
            if (ShotDistance > vec.magnitude)
            {
                ...
                if (Scnt % shotlate == 0)
                {
                    var s = Instantiate(NormalShot, this.transform.position + new Vector3(0, shellY, 0), Quaternion.identity);//弾を生成
                    float rad = R * Mathf.Deg2Rad;//°をradに変換
                    Vector3 direction = new Vector3(Mathf.Cos(rad), shellY, Mathf.Sin(rad));//力の方向を決める
                    Vector3 vet = direction * shotspeed;//力の大きさを決める
                    s.GetComponent<Rigidbody>().velocity = vet;//弾に力を加える
                    Scnt = 0;
                    R = R + shotlenge;
                    if (R >= 360)
                    {
