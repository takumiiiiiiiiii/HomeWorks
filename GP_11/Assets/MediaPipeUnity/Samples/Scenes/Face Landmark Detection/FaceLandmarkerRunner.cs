...
namespace Mediapipe.Unity.Sample.FaceLandmarkDetection
{
    public class FaceLandmarkerRunner : VisionTaskApiRunner<FaceLandmarker>
    {
        ...
        [SerializeField] private bool showCameraFeed = false;        // カメラ映像の表示ON/OFF
        [SerializeField] private bool showDetectionResults = false;  // 検出結果の表示ON/OFF
        ...
        private void OnFaceLandmarkDetectionOutput(FaceLandmarkerResult result, Image image, long timestamp)
        {
            ...
            {
                // Blendshapes リストを取得（NumFaces=1 前提）
                var faceBlendshapes = result.faceBlendshapes[0];     // 1 顔目の Blendshapes 情報

                // JawOpen に対応する値を取り出し
                var jawOpenCategory = faceBlendshapes.categories.FirstOrDefault(c => c.categoryName == "jawOpen");
                if (jawOpenCategory.categoryName != null) // null チェック
                {
                    float jawOpenScore = jawOpenCategory.score;

                    // スコアを閾値で判定してON/OFFに変換（0.3以上で口が開いていると判定）
                    bool isMouthOpen = jawOpenScore > 0.3f;

                    // bool値をfloat値に変換（0.0f = 閉じている, 1.0f = 開いている）
                    float mouthOpenValue = isMouthOpen ? 1.0f : 0.0f;

                    Debug.Log($"JawOpen Score: {jawOpenScore:F3}, Mouth Open: {isMouthOpen}");

                    // これをゲームロジックへ渡す
                    GameInput.Instance.SetMouthOpen(mouthOpenValue);
                }

                // // デバッグ用：すべてのカテゴリ名を表示
                // foreach (var category in faceBlendshapes.categories)
                // {
                //     Debug.Log($"Category: {category.categoryName}, Score: {category.score:F3}");
                // }
            }
        }
    }
